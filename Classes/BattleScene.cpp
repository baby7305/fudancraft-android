...
bool BattleScene::init(SocketClient* _socket_client, SocketServer* _socket_server)
{
    ...
    multi_touch_listener->onTouchesBegan = CC_CALLBACK_2(BattleScene::onTouchesBegan, this);
    multi_touch_listener->onTouchesMoved = CC_CALLBACK_2(BattleScene::onTouchesMoved, this);
	multi_touch_listener->onTouchesEnded = CC_CALLBACK_2(BattleScene::onTouchesEnded, this);
    //listener->onTouchesEnded = CC_CALLBACK_2(GameScene::onTouchesEnded, this);
    //listener->onTouchesCancelled = CC_CALLBACK_2(GameScene::onTouchesCancelled, this);
    Director::getInstance()->getEventDispatcher()->addEventListenerWithSceneGraphPriority(multi_touch_listener, this);
...
//	back_label->setFontSize(16);
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/battlefieldcontrol.mp3");
...
{
	//log("BattleScene: Entering BattleScene::update(), frame_cnt == %d", frame_cnt);
	frame_cnt++;
	//if (socket_client->error() || (socket_server&&socket_server->error()))
		//menuBackCallback(nullptr);
	//log("BattleScene: Check Error Success");
	//scrollMap();
	//log("BattleScene: Scroll Map Success");
	if (frame_cnt % KEY_FRAME == 0 && start_flag)
	{
		unit_manager->updateUnitsState();
	}
	//log("BattleScene: Completed BattleScene::update()");
}
...
void BattleScene::moveMap(Vec2 move_vec)
{
	auto map_center = battle_map->getPosition();
	auto visibleSize = Director::getInstance()->getVisibleSize();
	map_center += move_vec;
	if (battle_map->getBoundingBox().containsPoint(Vec2(0,0) - move_vec + visibleSize) && battle_map->getBoundingBox().containsPoint(Vec2(0, 0) - move_vec))
		battle_map->setPosition(map_center);
}
