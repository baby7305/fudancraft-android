...
#include <string>
#include "stdio.h"	//use function in stdio.h instead of std::to_string	
...
bool HelloWorld::init()
{
	...
	log("FudanCraft: HelloWorldScene Initialize Begin");
    
    auto visibleSize = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin(); 

	auto background0 = Sprite::create("BackgroundPanel.png");
	if (background0)
		log("FudanCraft: HelloWorldScene Read First Resource at Root Directory Success");
	else
		log("FudanCraft: HelloWorldScene Read First Resource at Root Directory Fail");
	
	//auto background1 = Sprite::create("//Picture//MenuItems//BackgroundPanel.png");
	//if (background1)
		//log("FudanCraft: HelloWorldScene Read First Resource with Special Path Success");
	//else
		//log("FudanCraft: HelloWorldScene Read First Resource with Special Path Fail");
	
	//auto background = Sprite::create("/Picture/MenuItems/BackgroundPanel.png");
	auto background = Sprite::create("Picture/MenuItems/BackgroundPanel.png");
	if (background)
		log("FudanCraft: HelloWorldScene Read First Resource Success");
	else
		log("FudanCraft: HelloWorldScene Read First Resource Fail");
	
	background->setPosition(origin + visibleSize / 2);
	log("FudanCraft: Set Position Success");
	background->setScaleX(visibleSize.width / background->getContentSize().width);
	log("FudanCraft: Set Scale Success");
	background->setScaleY(visibleSize.height / background->getContentSize().height);
	log("FudanCraft: Trying to Add Child");
	addChild(background);
	log("FudanCraft: Add First Child Success");
	
	//Modified Path: remove '/'
	...
	log("FudanCraft: Create First Sprite Success");
	...
	log("FudanCraft: HelloWorldScene Read Many Resources Success");
	...
	log("FudanCraft: Trying to Preload Audio");
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic("audio/mp3/killbill.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic("audio/mp3/mainv.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/bomb1.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/baseunderatack.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/battlefieldcontrol.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/selecttarget.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/unitlost.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/unitready.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/insufficientfound.mp3");
	CocosDenshion::SimpleAudioEngine::getInstance()->preloadEffect("audio/mp3/traning.mp3");

	log("FudanCraft: HelloWorldScene Initialize Success");
...
bool ServerMenu::init()
{
	...
	if (inputbox)
		...
		log("FudanCraft: Failed to Load InputPort.png");
...
void ServerMenu::menuStartServerCallback(cocos2d::Ref* pSender)
{
	if (!socket_server_)
	{
		log("FudanCraft: Trying to Start Server");
		socket_server_ = SocketServer::create();
		log("FudanCraft: SocketServer Created!");
		socket_client_ = SocketClient::create();
		log("FudanCraft: SocketClient Created!");
...
void ServerMenu::connectionSchdeule(float f)
{
		...
		//connection_msg_->setString("Total connection num: " + std::to_string(socket_server_->connection_num()));
	{
		...
		char con_num[10];
		sprintf(con_num, "%d", socket_server_->connection_num());
		std::string con_str(con_num);
		connection_msg_->setString("Total connection num: " + con_str);
...
bool CreditsScene::init()
{
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("audio/mp3/mainv.mp3",true);
