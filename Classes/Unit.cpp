...
void UnitManager::produceInBase(int _unit_type)
{
	...
	CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/traning.mp3");
...
void UnitManager::deleteUnit(int id)
{
	...
	if (unit)
	{
		...
		if(unit->getType()== 3) 
			...
			CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/die1.mp3");
		else
			//CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/bomb1.wav");
			CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/bomb1.mp3");
		
		if (notice && unit->camp == player_id)
		{
			//CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/unitlost.wav");
			CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/unitlost.mp3");
			char ntc[50];
...
Unit* UnitManager::createNewUnit(int id, int camp, int unit_type, GridPoint crt_gp)
{
	...
	char pic_chars[10];
	sprintf(pic_chars, "%d", pic_num);
	std::string pic_str(pic_chars);
	
	std::string pic_file = pic_paths[unit_type] + pic_str + suffixes[0];
	switch (unit_type)
	{
	case 1:
	...
	if (selected_ids.size())
	{
		...
		for (auto & id_unit : id_map)
			if (id_unit.second->camp == player_id && id_unit.second->getBoundingBox().containsPoint(select_point))
			{
				...
				CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/mp3/selecttarget.mp3");
